...
#Trying brute force technique, every point has four things(or less) to compair, down, right, left-diagonal, right-diagonal.  Max of 1600 comparisons
...
while ndx < 20:
	while mdx < 20:
#Check to make sure room to go right
		if mdx + 3 < 20:
			tprod = u[ndx][mdx]*u[ndx][mdx+1]*u[ndx][mdx+2]*u[ndx][mdx+3]
			if prod < tprod:
				prod = tprod
#Check to make sure room to go down or diagonal
		if ndx + 3 < 20:
			tprod = u[ndx][mdx]*u[ndx+1][mdx]*u[ndx+2][mdx]*u[ndx+3][mdx]
			if prod < tprod:
				prod = tprod
#Check for left diagonal
			if mdx - 3 >= 0:
				tprod = u[ndx][mdx]*u[ndx+1][mdx-1]*u[ndx+2][mdx-2]*u[ndx+3][mdx-3]
				if prod < tprod:
					prod = tprod
